...
public class Auto_BlueLeft extends AutonomousMethods {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        robot.followTrajectoryAsync(
                robot.trajectoryBuilder(PoseConstants.newBlueLeft.start)
                        .lineToSplineHeading(PoseConstants.newBlueLeft.backdrop[finalPropLocation - 1])
                        .build()
        );
        robot.waitForIdle();
        ...
        robot.followTrajectorySequenceAsync(
                robot.trajectorySequenceBuilder(robot.getPoseEstimate())
                        .forward(2)
                        .lineToSplineHeading(PoseConstants.newBlueLeft.pixel[finalPropLocation - 1])
                        .build()
        );
        roadrunnerSleep(500);
        resetSystem();
